...
const mockCategory = (type) => {
  return options[type][Math.floor(Math.random() * Object.keys(options).length)];
};
      ...
      <div className='absolute top-ml right bottom z1 w-full w240-ml px12 py12-ml'>
        <div className='flex-parent flex-parent--column viewport-third h-auto-ml hmax-full bg-white round-ml shadow-darken10'>
          <div className='px12 py12 scroll-auto'>
            <h3 className='txt-m txt-bold mb6'>{props.county.COUNTY} County</h3>
            <h4 className='txt-s txt-bold'>Most common opioid type: </h4>
            <h5 className='txt-xs mb6'>{props.county.opioidType || mockCategory('opioidType')}</h5>
          ...
          <footer className='px12 py12 bg-gray-faint round-b-ml txt-s'>
            Estimated cost: $USD {props.county.estimatedCost || (Math.random() * 2).toFixed(2)} Mil
          </footer>
        </div>
      </div>
    </div>
